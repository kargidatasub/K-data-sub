...
export default function AirtimeScreen() {
  ...
  // Load visibility & recent numbers on mount
  useEffect(() => {
    ...
    setTimeout(async () => {
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: `You successfully sent ₦${amount} airtime to ${phone} on ${network}.`,
          sound: true,
        },
        trigger: null, // Fire immediately
      });

      Alert.alert('Payment Successful!', `Successfully sent ₦${amount} airtime to ${phone}`, [
  ...
  return (
    <View style={[styles.container, isDark && styles.containerDark]}>
      <View style={[styles.topBar, { paddingTop: insets.top + 10 }]}>
        ...
        <Text style={[styles.topBarTitle, isDark && styles.textDark]}>Buy Airtime</Text>
        <View style={{ width: 24 }} /> 
      </View>
      ...
      >
        ...
        <Text style={[styles.sectionLabel, isDark && styles.textDark]}>Network Provider</Text>
        <View style={styles.simContainer}>
            {NETWORKS.map((net) => (
                <TouchableOpacity 
                    key={net.id} 
                    onPress={() => setNetwork(net.id)}
                    style={[styles.simCard, isDark && styles.simCardDark, network === net.id && styles.activeSimCard]}
                >
      ...
      {/* Glass Modal with Invisible PIN Input */}
      <Modal 
              ...
              <Animated.View style={[styles.modalWrapper, { opacity: modalOpacity, transform: [{ scale: modalScale }] }]}>
                  <BlurView intensity={Platform.OS === 'ios' ? 90 : 100} tint={isDark ? "dark" : "light"} style={styles.glassModal}>
                      ...
                      >
                        ...
                        <Text style={styles.modalSub}>Authorizing ₦{amount} airtime for {network}</Text>
...
const styles = StyleSheet.create({
  ...
  quickAmountChip: { width: '31%', backgroundColor: '#fff', paddingVertical: 12, borderRadius: 10, alignItems: 'center', marginBottom: 12, borderWidth: 1, borderColor: '#D1D5DB' },
  quickAmountChipDark: { backgroundColor: '#1F2937', borderColor: '#374151' },
  activeQuickAmount: { backgroundColor: '#0B2F66', borderColor: '#0B2F66' },
  quickAmountText: { fontSize: 14, fontWeight: 'bold', color: '#374151' },
  ...
  pinBoxesContainer: {
    flexDirection: 'row', justifyContent: 'center', gap: 15, marginVertical: 25, position: 'relative', width: '100%'
  },
  pinBox: {
    width: 55, height: 65, backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: 15, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: 'transparent'
  },
  ...
  cancelBtnText: { color: '#111827', fontWeight: 'bold' }, // <-- Added this!
